...
;(function() {
  ...
  var hashtagSpecialChars =
    '_\u200c\u200d\ua67e\u05be\u05f3\u05f4\u309b\u309c\u30a0\u30fb\u3003\u0f0b\u0f0c\u0f0d'

  // A hashtag must contain at least one unicode letter or mark, as well as numbers, underscores, and select special characters.
  twttr.txt.regexen.hashSigns = /[#＃]/
  twttr.txt.regexen.hashtagAlpha = new RegExp(
    '[' + unicodeLettersAndMarks + ']',
  )
  twttr.txt.regexen.hashtagAlphaNumeric = new RegExp(
    '[' + unicodeLettersAndMarks + unicodeNumbers + hashtagSpecialChars + ']',
  )
  twttr.txt.regexen.endHashtagMatch = regexSupplant(/^(?:#{hashSigns}|:\/\/)/)
  twttr.txt.regexen.hashtagBoundary = new RegExp(
    '(?:^|$|[^&' +
      unicodeLettersAndMarks +
      unicodeNumbers +
      hashtagSpecialChars +
  ...
  twttr.txt.regexen.validHashtag = regexSupplant(
    /(#{hashtagBoundary})(#{hashSigns})(#{hashtagAlphaNumeric}*#{hashtagAlpha}#{hashtagAlphaNumeric}*)/gi,
  )
  ...
  twttr.txt.regexen.validReply = regexSupplant(
    /^(?:#{spaces})*#{atSigns}([a-zA-Z0-9_]{1,20})/,
  )
  twttr.txt.regexen.endMentionMatch = regexSupplant(
  ...
  twttr.txt.regexen.validSpecialCCTLD = regexSupplant(
    RegExp('(?:(?:co|tv)(?=[^0-9a-zA-Z@]|$))'),
  )
  ...
  twttr.txt.regexen.invalidShortDomain = regexSupplant(
    /^#{validDomainName}#{validCCTLD}$/i,
  )
  ...
  twttr.txt.regexen.validUrlPathEndingChars = regexSupplant(
    /[\+\-a-z0-9=_#\/#{latinAccentChars}]|(?:#{validUrlBalancedParens})/i,
  )
  ...
  twttr.txt.regexen.validateUrlPath = regexSupplant(
    /(\/#{validateUrlPchar}*)*/i,
  )
  twttr.txt.regexen.validateUrlQuery = regexSupplant(
    /(#{validateUrlPchar}|\/|\?)*/i,
  )
  twttr.txt.regexen.validateUrlFragment = regexSupplant(
    /(#{validateUrlPchar}|\/|\?)*/i,
  )
  ...
  twttr.txt.tagAttrs = function(attributes) {
    ...
    for (var k in attributes) {
      ...
      htmlAttrs +=
        ' ' +
        twttr.txt.htmlEscape(k) +
        '="' +
        twttr.txt.htmlEscape(v.toString()) +
        '"'
    }
    return htmlAttrs
  }
  ...
  twttr.txt.linkToTextWithSymbol = function(
    entity,
    symbol,
    text,
    attributes,
    ...
    if (
      options.usernameIncludeSymbol ||
      !symbol.match(twttr.txt.regexen.atSigns)
    ) {
      return twttr.txt.linkToText(
        entity,
        taggedSymbol + taggedText,
        attributes,
        options,
      )
    } else {
      return (
        taggedSymbol +
        twttr.txt.linkToText(entity, taggedText, attributes, options)
      )
    }
  }
  ...
  twttr.txt.linkTextWithEntity = function(entity, options) {
    ...
    if (expandedUrl.indexOf(displayUrlSansEllipses) != -1) {
      ...
      var v = {
        ...
        afterDisplayUrl: expandedUrl.substr(
          displayUrlIndex + displayUrlSansEllipses.length,
        ),
  ...
  twttr.txt.autoLinkEntities = function(text, entities, options) {
    ...
    options.hashtagUrlBase =
      options.hashtagUrlBase || 'https://twitter.com/#!/search?q=%23'
    options.cashtagClass = options.cashtagClass || DEFAULT_CASHTAG_CLASS
    options.cashtagUrlBase =
      options.cashtagUrlBase || 'https://twitter.com/#!/search?q=%24'
  ...
  twttr.txt.extractEntitiesWithIndices = function(text, options) {
    var entities = twttr.txt
      ...
      .concat(
        twttr.txt.extractHashtagsWithIndices(text, { checkUrlOverlap: false }),
      )
  ...
  twttr.txt.extractMentionsOrListsWithIndices = function(text) {
    ...
    text.replace(twttr.txt.regexen.validMentionOrList, function(
      match,
      before,
      atSign,
      screenName,
      ...
      if (!after.match(twttr.txt.regexen.endMentionMatch)) {
        ...
        var endPosition =
          startPosition + screenName.length + slashListname.length + 1
        possibleNames.push({
    ...
    while (twttr.txt.regexen.extractUrl.exec(text)) {
      ...
      if (!protocol) {
        if (
          !options.extractUrlsWithoutProtocol ||
          before.match(
            twttr.txt.regexen.invalidUrlWithoutProtocolPrecedingChars,
          )
        ) {
          continue
        }
        var lastUrl = null,
          asciiEndPosition = 0
        domain.replace(twttr.txt.regexen.validAsciiDomain, function(
          asciiDomain,
        ) {
  ...
  twttr.txt.extractHashtagsWithIndices = function(text, options) {
    ...
    text.replace(twttr.txt.regexen.validHashtag, function(
      match,
      before,
      hash,
      hashText,
  ...
  twttr.txt.extractCashtagsWithIndices = function(text) {
    ...
    text.replace(twttr.txt.regexen.validCashtag, function(
      match,
      before,
      dollar,
      cashtag,
  ...
  twttr.txt.getTweetLength = function(text, options) {
    ...
    for (var i = 0; i < urlsWithIndices.length; i++) {
      ...
      textLength +=
        urlsWithIndices[i].indices[0] - urlsWithIndices[i].indices[1]
  ...
  twttr.txt.isValidUrl = function(url, unicodeDomains, requireProtocol) {
    ...
    return (
      (unicodeDomains &&
        isValidMatch(
          authority,
          twttr.txt.regexen.validateUrlUnicodeAuthority,
        )) ||
      (!unicodeDomains &&
        isValidMatch(authority, twttr.txt.regexen.validateUrlAuthority))
    )
  }
